...
let mapOrigin = {x: 175, y: 175};
//How large to make the minimap
let miniMapRadius = 100;
...
function DrawWalls()
{
	let map_data = parent.G.maps[character.map].data;
    for (id in map_data.x_lines) {
		...
		let localPoint1 = WorldToLocal(x1, y1, scale);
		let localPoint2 = WorldToLocal(x2, y2, scale);
...
{
	let intersections = getIntersections([point1.x, point1.y], [point2.x, point2.y], [0,0,miniMapRadius - miniMapBorder/2]);
		
		let miniMapPoint1 = null;
		let miniMapPoint2 = null;
		...
		if(intersections.points != false)
		{
			...
			{
				let intersect = intersections.points[id];
				
				if(intersect.onLine)
				{
					let point = {x: intersect.coords[0], y: intersect.coords[1]};
...
{
	let localPosition = WorldToLocal(mapOrigin.x, mapOrigin.y, 1);
	let color=0x47474F;
	let size= miniMapBorder;
	e.lineStyle(size, color);
	e.beginFill(0x40420);
...
function DrawEntities()
{
	for(id in parent.entities)
	{
		...
		let localPos = WorldToLocal(entity.real_x, entity.real_y, scale);
		
		if(miniMapDistance(localPos.x, localPos.y) < miniMapRadius - miniMapBorder)
		{
			let color=EntityColorMapping(entity);
			let fill = color;
			let size=2;
			let borderSize = 2;
...
function DrawNPCs()
{
	...
	{
		let npc = parent.G.maps[character.map].npcs[id];
		...
		{
			let localPos = WorldToLocal(position[0], position[1], scale);
...
{
	let relativeX = (character.real_x - x)*scale * -1;
	let relativeY = (character.real_y - y)*scale * -1;
