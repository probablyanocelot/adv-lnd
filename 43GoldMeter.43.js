...
function updateGoldTimerList()
{
	...
	let goldString = "<div>" + gold + " Gold/Hr" + "</div>"; 
...
function getGold()
{
	...
	let goldPerSecond = parseFloat(Math.round((sumGold/(elapsed/1000)) * 100) / 100);
...
function goldMeterGameLogHandler(event)
{
	if(event.color == "gold")
	{
		let gold = parseInt(event.message.replace(" gold", "").replace(",", ""));
